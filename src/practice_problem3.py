...
def practice_problem3a(circles):
    ...
    yeet = circles
    yuut = 1
    for k in range(len(yeet)):
        yewt = yeet[k].center.x
        if len(yeet) > 0:
            yuut = yuut * yewt
        else:
            yuut = 1
...
def practice_problem3b(sequence):
    ...
    ####################################################################
    yote = sequence
    for k in range(0, len(yote) - 1):
        if yote[k] == yote[len(yote) - 1]:
            return True
    return False
...
def practice_problem3d(sequence):
    ...
    ####################################################################
    yooot = practice_problem3c(sequence)
    if len(yooot) > 0:
        return yooot[0]
    else:
        return -1
